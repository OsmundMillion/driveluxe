  ...
  return (
    <section className="py-12 bg-midnight text-white px-4 sm:px-6 lg:px-12">
      ...
      <h2 className="text-4xl font-semibold text-center text-white mb-8">
        <span className="text-white">Our</span> <span className="text-luxeGold">Services</span>
      </h2>
        ...
        <AnimatePresence mode="wait">
          {(activeSet === 1 ? servicesSet1 : servicesSet2).map((service) => (
            <motion.div
              ...
              className="relative bg-[#292929] p-0 rounded-2xl overflow-hidden shadow-lg hover:translate-y-4 transition-all duration-300 cursor-pointer hover:ring-2 hover:ring-luxeGold"
            >
              <div className="absolute left-4 top-4 w-10 h-10 bg-luxeGold rounded-full flex items-center justify-center text-midnight font-bold text-lg shadow-md animate-pulse">
      ...
      <div className="flex justify-center mt-10 space-x-3">
        <motion.button
          ...
          className={`w-6 h-6 rounded-full border-1 border-luxeGold ${activeSet === 1 ? "bg-luxeGold" : "bg-transparent"}`}
        />
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={() => handleSetClick(2)}
          className={`w-6 h-6 rounded-full border-1 border-luxeGold ${activeSet === 2 ? "bg-luxeGold" : "bg-transparent"}`}
        />
      </div>
    </section>
  );
