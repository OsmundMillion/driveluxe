...
import { cars } from "../data/carsData"; // adjust path if needed

const FeaturedCars = () => {
  const navigate = useNavigate();
  const [currentSlide, setCurrentSlide] = useState(0);

  const featuredCars = cars.slice(0, 9);
  const slidesPerView = 3;
  const maxSlide = Math.ceil(featuredCars.length / slidesPerView) - 1;
      ...
      <div className="relative w-screen flex items-center justify-center px-6">
        ...
        <div className="overflow-hidden w-full">
          <div
            ...
            style={{
              width: `${(featuredCars.length / slidesPerView) * 100}%`,
              transform: `translateX(-${(100 / (featuredCars.length / slidesPerView)) * currentSlide}%)`,
            }}
          >
              ...
              <div key={car.id} className="w-[33.3333vw] px-2 shrink-0">
                <div className="bg-[#1a1a1a] rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl hover:ring-1 hover:ring-luxeGold transition-transform duration-300 transform hover:scale-105">
                  ...
                  <div className="p-4">
                    ...
                    <p className="mt-1 text-sm text-slate-300 font-light">${car.price}/day</p>
                    <button
